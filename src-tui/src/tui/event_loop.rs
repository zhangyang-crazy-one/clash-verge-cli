...
/// Task 8.1: persist the edited DNS spec to the TUI-owned sidecar file.
fn persist_dns_spec(app: &App) -> Result<(), String> {
                            ...
                            InputMode::RuleFormInput(buffer) => {
                                match key.code {
                                        ...
                                        if !spec.is_empty() && app.rules_edit_mode {
                                            match crate::routing::build_simple_rule(&spec)
                                            {
                                                Ok(rule) => {
                                                    ...
                                                    app.status_msg =
                                                        Some(format!("rule built: {spec}"));
                                                }
                                                Err(e) => {
                                                    app.status_msg =
                                                        Some(format!("invalid form: {e}"));
                                                }
                                            }
                                        }
                                        app.input_mode = InputMode::Normal;
                                    ...
                                    KeyCode::Backspace => {
                                        let mut b = buffer.clone();
                                        b.pop();
                                        app.input_mode = InputMode::RuleFormInput(b);
                                    }
                                    KeyCode::Char(c) => {
                                        let mut b = buffer.clone();
                                        b.push(c);
                            ...
                            InputMode::DnsServerInput(buffer) => {
                                match key.code {
                                    ...
                                    KeyCode::Enter => {
                                        let spec = buffer.trim().to_string();
                                        if !spec.is_empty() {
                                            match crate::singbox::dns::parse_server_spec(
                                                &spec,
                                            ) {
                                                Ok(server) => {
                                                    let tag = server.tag.clone();
                                                    app.dns_spec_edit.servers.push(server);
                                                    if let Err(e) = persist_dns_spec(&app) {
                                                        app.dns_spec_edit.servers.pop();
                                                        app.status_msg =
                                                            Some(format!("persist dns: {e}"));
                                                    } else {
                                                        app.status_msg =
                                                            Some(format!("dns server added: {tag}"));
                                                    }
                                                }
                                                Err(e) => {
                                                    app.status_msg =
                                                        Some(format!("invalid dns server: {e}"));
                                                }
                                            }
                                        }
                                        app.input_mode = InputMode::Normal;
                                    ...
                                    KeyCode::Backspace => {
                                        let mut b = buffer.clone();
                                        b.pop();
                                        app.input_mode = InputMode::DnsServerInput(b);
                                    }
                                    KeyCode::Char(c) => {
                                        let mut b = buffer.clone();
                                        b.push(c);
                            ...
                            InputMode::DnsRuleInput(buffer) => {
                                match key.code {
                                            ...
                                            match crate::singbox::dns::parse_rule_spec(&spec)
                                            {
                                                Ok(rule) => {
                                                    ...
                                                    if let Err(e) = persist_dns_spec(&app) {
                                                        ...
                                                        app.status_msg =
                                                            Some(format!("dns rule added -> {target}"));
                                                    }
                                                }
                                                Err(e) => {
                                                    app.status_msg =
                                                        Some(format!("invalid dns rule: {e}"));
                                                }
                                            }
                                        }
                                        app.input_mode = InputMode::Normal;
                                    ...
                                    KeyCode::Backspace => {
                                        let mut b = buffer.clone();
                                        b.pop();
                                        app.input_mode = InputMode::DnsRuleInput(b);
                                    }
                                    KeyCode::Char(c) => {
                                        let mut b = buffer.clone();
                                        b.push(c);
                                    ...
                                    KeyCode::Enter => {
                                        ...
                                        if tag.is_empty() {
                                            ...
                                            match persist_dns_spec(&app) {
                                                Ok(()) => {
                                                    app.status_msg =
                                                        Some("domain_resolver cleared".into());
                                                }
                                                Err(e) => {
                                                    app.status_msg =
                                                        Some(format!("persist dns: {e}"));
                                            ...
                                            .any(|s| s.tag == tag)
                                        {
                                            ...
                                            match persist_dns_spec(&app) {
                                                Ok(()) => {
                                                    app.status_msg =
                                                        Some(format!("domain_resolver set: {tag}"));
                                                }
                                                Err(e) => {
                                                    app.status_msg =
                                                        Some(format!("persist dns: {e}"));
                                            ...
                                            app.status_msg =
                                                Some(format!("unknown server tag: {tag}"));
                                        }
                                        app.input_mode = InputMode::Normal;
                                    }
                                    KeyCode::Backspace => {
                                        let mut b = buffer.clone();
                                        b.pop();
                                        app.input_mode = InputMode::DnsResolverInput(b);
                                    }
                                    KeyCode::Char(c) => {
                                        let mut b = buffer.clone();
                                        b.push(c);
                                            ...
                                            match app.view {
                                                ...
                                                View::Settings => {
                                                    if app.dns_edit_mode {
                                                        ...
                                                        if len > 0 {
                                                            app.dns_cursor =
                                                                (app.dns_cursor + len - 1) % len;
                                                        }
                                                    } else {
                                                        let count =
                                                            crate::ui::views::settings::SETTINGS_ROW_COUNT;
                                                        app.settings_selected_index =
                                                            (app.settings_selected_index + count - 1)
                                                                % count;
                                                    }
                                                }
                                                _ => {}
            ...
            action = action_rx.recv() => {
                match action {
                                            ...
                                            match std::fs::read_to_string(&path) {
                                                Ok(yaml) => match crate::routing::load_profile_rules(&yaml) {
                                                    Ok(rules) => {
                                                        ...
                                                        app.rules_edit_dirty = false;
                                                        app.status_msg =
                                                            Some("rule editing ON - D del, J/K move, A raw, F form, W save, E exit".into());
                                                    }
                    ...
                    Some(Action::RulesEditSave) if app.rules_edit_mode && app.rules_edit_dirty => {
                        // Task 7.5: sing-box cannot hot-reload route rules -
                        // make the restart cost explicit before batch-saving.
                        if manager.core_kind() == crate::mihomo_manager::CoreKind::SingBox {
                            app.overlay = Some(Overlay::RulesRestartConfirmation);
                            app.status_msg =
                                Some("saving will regenerate config & restart sing-box".into());
                        } else {
                    ...
                    Some(Action::RulesEditSaveCancelled)
                        if app.overlay == Some(Overlay::RulesRestartConfirmation) =>
                    {
                        // Edits stay in the buffer so the user can keep working.
                        app.overlay = None;
                        app.status_msg = Some("save cancelled - edits kept".into());
                    }
                    Some(Action::DnsEditToggle) => {
                        if app.dns_edit_mode {
                            ...
                            app.status_msg = Some(
                                "sing-box DNS editor ON - a server | r rule | R resolver | x del | Tab lists | w apply | d exit"
                                    .into(),
                            );
                        }
                    }
                    Some(Action::DnsFocusToggle) if app.dns_edit_mode => {
                    ...
                    Some(Action::DnsAddServer) if app.dns_edit_mode => {
                        app.input_mode = InputMode::DnsServerInput(String::new());
                    }
                    Some(Action::DnsAddRule) if app.dns_edit_mode => {
                        app.input_mode = InputMode::DnsRuleInput(String::new());
                    }
                    Some(Action::DnsSetResolver) if app.dns_edit_mode => {
                        let current =
                            app.dns_spec_edit.domain_resolver.clone().unwrap_or_default();
                        app.input_mode = InputMode::DnsResolverInput(current);
                    }
                    Some(Action::DnsDeleteEntry) if app.dns_edit_mode => {
                        let len = dns_list_len(&app);
                        let removed_msg = if app.dns_cursor >= len {
                            None
                        } else if app.dns_focus_rules {
                        ...
                        match removed_msg {
                            Some(msg) => {
                                app.dns_cursor =
                                    app.dns_cursor.min(dns_list_len(&app).saturating_sub(1));
                                if let Err(e) = persist_dns_spec(&app) {
                                    app.status_msg = Some(format!("persist dns: {e}"));
                                } else {
                                    app.status_msg = Some(msg);
                                }
                            }
                    ...
                    Some(Action::DnsApply) if app.dns_edit_mode => {
                        if manager.core_kind() != crate::mihomo_manager::CoreKind::SingBox {
                            app.status_msg =
                                Some("switch proxy core to sing-box before applying DNS".into());
                        } else {
                            let m = manager.clone();
                            let tx = action_tx.clone();
                            tokio::spawn(async move {
                                ...
                                {
                                    Ok(msg) => {
                                        let _ = tx.send(Action::DnsApplied(msg));
                                    }
                                    Err(e) => {
                                        let _ = tx.send(Action::DnsApplyFailed(e));
                                    }
                                }
                            });
                            app.status_msg =
                                Some("regenerating config & restarting sing-box...".into());
                        }
                    }
                    Some(Action::DnsApplied(msg)) => {
                        app.status_msg = Some(msg);
                    }
                    Some(Action::DnsApplyFailed(error)) => {
